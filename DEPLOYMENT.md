...
Get the Backend URL
After successful deployment, Google will provide a URL (e.g., https://ai-marketing-backend-xyz.a.run.app).
...
Select your repository, branch (main), and the main file path: frontend/app.py.
...
[general]
API_BASE_URL = "[https://ai-marketing-backend-xyz.a.run.app](https://ai-marketing-backend-xyz.a.run.app)"
...
Example: API_BASE_URL = st.secrets.get("general", {}).get("API_BASE_URL", "http://127.0.0.1:8000")
...
503 Service Unavailable: Usually means the Backend container crashed on startup. Check Cloud Run logs. Common cause: Missing requirements.txt packages or invalid .env variables.

Frontend cannot connect: Ensure the Backend URL in Streamlit Secrets does not have a trailing slash (/) unless your code expects it, and that the Backend allows CORS from the Streamlit domain.